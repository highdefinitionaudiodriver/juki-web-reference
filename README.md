...
| Spring テスト | MockMvc + Testcontainers IT 94 件（Docker なし環境では IT 19 件 skip） |
...
| レイヤ | テストランナ | 件数 | 内訳 |
|---|---|---:|---|
| Spring API Controller | MockMvc | 68 | Auth(3) / Resident(8) / ResidentSupplement(5) / Transaction(13) / Certificate(8) / Restriction(6) / Report(4) / Link(7) / Admin(7) / Audit(2) / EUC(5) |
...
**合計 158 ケースを CI で自動検証**。Linux runner + Docker 環境では IT も含めて全件 PASS。

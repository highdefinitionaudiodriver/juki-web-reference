...
| Spring テスト | MockMvc + Testcontainers IT 89 件（Docker なし環境では IT 17 件 skip） |
...
| Spring Integration | Testcontainers + Spring Boot | 17 | ResidentApiIT(2) / HouseholdSplitMergeIT(2) / KosekiIT(5) / AdminIT(4) / RestrictionIT(2) / EucIT(2) — Linux runner + Docker で実行、ローカル Docker なしは skip |
...
**合計 153 ケースを CI で自動検証**。Linux runner + Docker 環境では IT も含めて全件 PASS。

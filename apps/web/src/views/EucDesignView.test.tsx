...
describe("EucDesignView (SCR-A01 EUC設計)", () => {
  it("登録済みテンプレートと承認区分を表示する", async () => {
    render(<EucDesignView loadTemplates={vi.fn().mockResolvedValue(templates)} onCreate={vi.fn()} onDelete={vi.fn()} />);
  ...
  it("個人番号を選ぶと二人承認の注意を表示する", async () => {
    render(<EucDesignView loadTemplates={vi.fn().mockResolvedValue([])} onCreate={vi.fn()} onDelete={vi.fn()} />);
  ...
  it("保存で onCreate が呼ばれる", async () => {
    ...
    render(<EucDesignView loadTemplates={vi.fn().mockResolvedValue([])} onCreate={onCreate} onDelete={vi.fn()} />);
    const user = userEvent.setup();
